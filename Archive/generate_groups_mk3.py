...
- Athlete_Groups - Data.csv   : Performance data for all athletes
- Roster.csv                  : Full team roster (optional but recommended)
- athlete_groups.html         : Existing groups (optional, for preserving manual entries)

OUTPUTS:
- athlete_groups.html         : Updated training groups with VDOT toggle
- vdot_verification.csv       : VDOT calculations for verification

HOW IT WORKS:
1. Loads VDOT table and athlete performance data
2. Creates groups based on VDOT fitness levels
3. Preserves manually-added athletes from existing HTML (except Unassigned group)
4. Checks Roster.csv and adds unassigned athletes to final group
5. Prevents duplicates by tracking all assigned names
"""
...
def merge_with_existing(new_groups, existing_groups, athletes_with_data):
    ...
    Skip the highest numbered group (assumed to be Unassigned) to avoid duplicates.
    """
    ...
    # Find the highest group number (likely the Unassigned group)
    max_group_num = max(existing_groups.keys()) if existing_groups else 0
    
    for group_num, athletes in existing_groups.items():
        # Skip the highest numbered group - we'll regenerate it from roster
        if group_num == max_group_num and len(existing_groups) > 1:
            continue
    ...
    # Track all names already in merged groups to prevent duplicates
    existing_names_in_merged = set()
    for group_athletes in merged_groups.values():
        for athlete in group_athletes:
            existing_names_in_merged.add(athlete['name'])
    
    for group_num, athletes in preserved_athletes.items():
        if group_num not in merged_groups:
            merged_groups[group_num] = []
        for athlete_name in athletes:
            # Only add if not already in merged groups
            if athlete_name not in existing_names_in_merged:
                merged_groups[group_num].append({'name': athlete_name, 'manual': True})
                existing_names_in_merged.add(athlete_name)
...
def main():
    ...
    if ROSTER_PATH.exists():
        ...
        print(f"   ℹ️  Roster.csv not found in script directory")
        print(f"   ℹ️  Place Roster.csv in same folder as script to check for unassigned athletes")
    ...
    if roster_names:
        ...
        # Collect all assigned athlete names from BOTH data and manual entries
        assigned_names = set()
        for group_num, group_athletes in merged_groups.items():
            for athlete in group_athletes:
                assigned_names.add(athlete['name'])
        
        # Find unassigned athletes (in roster but not assigned anywhere)
        ...
        if unassigned_names:
            ...
            for name in sorted(unassigned_names):
                merged_groups[unassigned_group_num].append({
                    'name': name,
                    'manual': True,
                    'unassigned': True  # Flag for special handling
                })
                print(f"      - {name}")
        else:
            print(f"   ✓ All roster athletes accounted for in groups")
    else:
        print(f"   ℹ️  No roster check performed (Roster.csv not found in script directory)")
    ...
    for group_num in sorted(merged_groups.keys()):
        ...
        if vdots:
            ...
            vdot_range = "Manual entries only" if not is_unassigned else "No data available"
