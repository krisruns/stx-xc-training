...
Now with BUILT-IN PRINT FUNCTIONALITY for group record sheets!
"""
...
def process_training_overview(overview_csv, paces_csv, athlete_groups_html, output_dir='output'):
    ...
    for idx, row in df.iterrows():
        ...
        for day in days:
            ...
            print(f"  ✓ {output_file.name:40s} [{pace_summary}] + PRINT BUTTON")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Generate workout-specific pace pages with PRINTABLE GROUP SHEETS'
    )
    parser.add_argument(
        'overview_csv',
        nargs='?',
