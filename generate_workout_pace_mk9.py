...
def load_training_paces(csv_path='training_paces.csv'):
    ...
    pace_columns = {
        ...
        # Pre 30/40 paces
        'pre_200': 13,      # kept for backward compat
        'pre30_200': 13,    # Pre 30 — faster 200m rep
        'pre40_200': 14,    # Pre 40 — slower 200m rep
...
def parse_workout_description(description):
    ...
    # ── Pre Tempo / Pre 30/40 detection ─────────────────────────────────────
    # Matches: "Pre Tempo", "Pre 30/40", "Pre 30", "pre-tempo", etc.
    if re.search(r'pre[\s\-]*(tempo|30(?:/40)?)', description, re.IGNORECASE):
        return [('200', 'pre30'), ('200', 'pre40')]
    # ────────────────────────────────────────────────────────────────────────
    ...
    for reps, bracketed_content in bracket_matches:
        # Parse time-based patterns inside brackets: 20min@T, 20min@Tempo
        time_inner_pattern = r'(\d+)min@([\w\d]+)'
        time_inner_matches = re.findall(time_inner_pattern, bracketed_content)
        for duration, pace_type in time_inner_matches:
            pace_needs.append(('mile', pace_type))

        # Parse all distance-based pace patterns inside the brackets
        inner_pattern = r'(\d+)(?:m)?@([\w\d]+)'
        inner_matches = re.findall(inner_pattern, bracketed_content)
        for distance, pace_type in inner_matches:
            # Skip matches that are part of a "NNmin@" pattern (already handled above)
            if (distance, pace_type) not in [(d, p) for d, p in time_inner_matches]:
    ...
    # Handle time-based workouts: 20min@T, 20min@Tempo, 30min@CV, etc.
    # These use a duration instead of a rep distance; look up the per-mile pace.
    time_pattern = r'(\d+)\s*min@([\w\d]+)'
    time_matches = re.findall(time_pattern, desc_without_brackets)
    ...
    # Distance can be numeric (400, 1600) OR a word (mile, km)
    pattern = r'(\d+)x([a-zA-Z\d]+)(?:m)?@([\w\d]+)'
    matches = re.findall(pattern, desc_without_brackets)
    
    for match in matches:
        reps, distance, pace_type = match
        # Strip trailing 'm' from numeric distances (e.g. '1600m' → '1600')
        if distance.endswith('m') and distance[:-1].isdigit():
            distance = distance[:-1]
        pace_needs.append((distance, pace_type))
...
def get_pace_range_for_group(vdot_paces, athletes, distance, pace_type):
    ...
    pace_aliases = {
        ...
        '10k': 'cv',
        'cv': 'cv',
        'pre30': 'pre30',   # Pre 30/40 — faster 200m rep
        'pre40': 'pre40',   # Pre 30/40 — slower 200m rep
    }
    
    normalized_pace = pace_aliases.get(pace_type.lower(), pace_type.lower())
    
    # Normalize distances: some pace table keys use names instead of meters
    # e.g., tempo_mile instead of tempo_1600, mile_200 refers to the pace not the rep distance
    distance_name_map = {
        '1600': 'mile',
        '1609': 'mile',
        'mile': 'mile',
    }
    normalized_distance = distance_name_map.get(distance.lower(), distance)
    lookup_key = f"{normalized_pace}_{normalized_distance}"
...
def add_paces_to_groups(groups, pace_needs, vdot_paces):
    ...
    for group in groups:
        for distance, pace_type in pace_needs:
            ...
            # Create pace label — normalize distance display
            # 'mile' distance means a per-mile pace lookup (used for timed/mileage workouts)
            if distance.lower() == 'mile':
                pace_label = f"/mile @ {pace_type.upper()}"
            elif pace_type.lower() == 'pre30':
                pace_label = "200m Rep 1 (Pre 30)"
            elif pace_type.lower() == 'pre40':
                pace_label = "200m Rep 2 (Pre 40)"
            else:
