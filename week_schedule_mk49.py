...
WEEK_8_START = datetime(2025, 7, 14)
...
def main():
    ...
    # Parse CSV
    workouts = []
    week_start_iso = None  # will be populated from CSV if present
    with open(input_file) as f:
        reader = csv.DictReader(f)
        for row in reader:
            # Grab the Week_Start date from the first row that has it
            ...
            # If workout is "Off", clear mileage and pre/post work
    ...
    # Calculate date range for heading (prefer CSV Week_Start, fall back to calculation)
    ...
    # Write output
    output_file = input_file.stem + '.html'
