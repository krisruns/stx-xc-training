...
# Configuration: Directory where pace page files are stored
PACE_PAGES_DIR = 'pace_pages'

# Anchor: Week 8 starts Monday July 14, 2025
WEEK_8_START = datetime(2026, 7, 13)


def get_week_date_range(week_num, week_start_iso=None):
    """
    Return a formatted date range string including year.
    If week_start_iso (YYYY-MM-DD) is provided, use it directly.
    Otherwise fall back to calculating from the Week 8 anchor.
    Examples:
        "April 13 - 19, 2026"
    ...
    try:
        if week_start_iso:
            start = datetime.strptime(week_start_iso.strip(), '%Y-%m-%d')
        else:
            start = WEEK_8_START + timedelta(weeks=(int(week_num) - 8))
    except (ValueError, TypeError):
        return None
    end = start + timedelta(days=6)
    year = end.year  # use end date's year (handles Dec 28 - Jan 3 edge case)
    if start.month == end.month:
        return f"{start.strftime('%B %-d')} - {end.strftime('%-d')}, {year}"
    else:
        return f"{start.strftime('%B %-d')} - {end.strftime('%B %-d')}, {year}"
...
def has_parseable_paces(workout_desc):
    ...
    Returns True if workout contains patterns like:
    - "12x400@5k", "2x200@mile" (simple rep patterns)
    - "3x[600@3k + 400@mile + 200@800]" (bracketed patterns)
    - "20min@T", "20min@Tempo" (time-based patterns)
    - "2miles@T", "2 miles@Tempo" (distance-in-miles patterns)
    """
    if not workout_desc:
        return False

    desc_lower = workout_desc.lower()

    # Bracketed patterns: 3x[...]
    if re.search(r'\d+x\[.*?@.*?\]', desc_lower):
        return True

    # Time-based: 20min@T, 20min@Tempo
    if re.search(r'\d+\s*min@[\w\d]+', desc_lower):
        return True

    # Miles-based: 2miles@T, 2 miles@Tempo, 2.5miles@CV
    if re.search(r'\d+(?:\.\d+)?\s*miles?@[\w\d]+', desc_lower):
        return True

    # Simple rep patterns: 12x400@5k, 5xmile@T
    if re.findall(r'\d+x[a-z\d]+(?:m)?@[\w\d]+', desc_lower):
        return True
...
def generate_html(week_num, workouts, totals, library, date_range=None):
    ...
    for day, status, group, pre, workout, post, miles, groups_file, workout_id in workouts:
        ...
        if day_title not in day_groups_files or day_groups_files[day_title] == 'athlete_groups.html':
            ...
            if has_parseable_paces(workout):
                # Try to find the pace file - check multiple possibilities
                statuses_to_try = []
                if status == 'All' or not status:
                    statuses_to_try = ['Varsity', '']  # Try Varsity first, then no prefix
                else:
                    statuses_to_try = [status]
                
                for try_status in statuses_to_try:
                    test_filename = get_workout_pace_filename(day_title, week_num, try_status)
                    # Check if file exists in pace_pages directory
                    pace_file_path = join(PACE_PAGES_DIR, test_filename)
                    if exists(pace_file_path):
                        day_groups_files[day_title] = pace_file_path  # Store full path with folder
...
<body>
    ...
    <h1>🏃 STX Training - Week {week_num}{f" — {date_range}" if date_range else ""}</h1>
...
{chr(10).join(totals_html)}
    ...
    <div class="filters">
        ...
        <button class="filter-btn" onclick="filterGroup('Blue')">Blue</button>
    </div>
    ...
    <div class="day">
        ...
        for status in ['Varsity', 'JV', 'All']:
            ...
            for group, pre, workout, post, miles, workout_id in sorted(workouts_list, key=lambda x: groups.index(x[0]) if x[0] in groups else 999):
                ...
                if has_parseable_paces(workout):
                    # Try to find the pace file - check multiple possibilities
                    # If Status is 'All' or empty, workouts are typically Varsity by default
                    statuses_to_try = []
                    if status == 'All' or not status:
                        statuses_to_try = ['Varsity', '']  # Try Varsity first, then no prefix
                    else:
                        statuses_to_try = [status]  # Use specific status
                    
                    pace_filename = None
                    for try_status in statuses_to_try:
                        test_filename = get_workout_pace_filename(day, week_num, try_status)
                        # Check if file exists in pace_pages directory
                        pace_file_path = join(PACE_PAGES_DIR, test_filename)
                        if exists(pace_file_path):
                            pace_filename = pace_file_path  # Store full path with folder
                            break
...
def process_one_week(week_num_str, library, day_map, schedules_dir, output_dir):
    """Process a single week CSV and write the HTML output."""

    # Support both zero-padded (week01.csv) and plain (week1.csv)
    padded = Path(schedules_dir) / f'week{int(week_num_str):02d}.csv'
    plain  = Path(schedules_dir) / f'week{int(week_num_str)}.csv'
    input_file = padded if padded.exists() else plain if plain.exists() else None

    if input_file is None:
        print(f"  ⚠ week{week_num_str}: CSV not found in {schedules_dir} — skipping")
        return False
    ...
    with open(input_file) as f:
        ...
        for row in reader:
            if week_start_iso is None:
                week_start_iso = row.get('Week_Start', '').strip() or None
    ...
    if not workouts:
        print(f"  ⚠ week{week_num_str}: no valid workouts found — skipping")
        return False

    # Use stem digits as the canonical week number for heading/links
    week_num = ''.join(c for c in input_file.stem if c.isdigit()) or week_num_str

    date_range = get_week_date_range(week_num, week_start_iso)
    totals     = calculate_group_totals(workouts)
    html       = generate_html(week_num, workouts, totals, library, date_range)

    # Output to the specified directory (or same as input if not set)
    out_dir  = Path(output_dir) if output_dir else Path(__file__).parent
    out_dir.mkdir(parents=True, exist_ok=True)
    out_file = out_dir / (input_file.stem + '.html')

    with open(out_file, 'w') as f:
        f.write(html)

    linked = sum(1 for *_, wid in workouts if wid)
    total_str = ', '.join(
        f"{g} {totals.get(g, (0,0))[0]:.0f}mi"
        for g in ['Gold', 'Green', 'White', 'Blue']
        if totals.get(g, (0,0))[0] > 0
    )
    print(f"  ✓ week{week_num:>2}  →  {out_file.name}  [{total_str}]  ({linked} linked workouts)")
    return True
...
def main():
    """
    Usage:
        python week_schedule_mk49.py                  # prompts for week number
        python week_schedule_mk49.py 5                # single week
        python week_schedule_mk49.py 1 23             # range: weeks 1 through 23
        python week_schedule_mk49.py week05.csv       # explicit CSV file path
        python week_schedule_mk49.py 1 23 --out .     # range with custom output dir
    """
    import argparse

    parser = argparse.ArgumentParser(
        description='Generate STX weekly training HTML from weekly schedule CSVs.'
    )
    parser.add_argument('weeks', nargs='*',
        help='Week number(s): a single number (5), a range start + end (1 23), or a CSV path')
    parser.add_argument('--schedules-dir', default='weekly_schedules',
        help='Folder containing weekNN.csv files (default: weekly_schedules)')
    parser.add_argument('--library', default=None,
        help='Path to workout_library.csv (auto-detected if omitted)')
    parser.add_argument('--out', default=None,
        help='Output directory for HTML files (default: same as CSV)')
    args, unknown = parser.parse_known_args()

    # Determine week range / single file
    weeks_arg = args.weeks

    # Prompt if nothing provided
    if not weeks_arg:
        raw = input("Enter week number (or start end for a range): ").strip()
        weeks_arg = raw.split()

    # Single CSV file path passed?
    if len(weeks_arg) == 1 and not weeks_arg[0].isdigit():
        # Treat as direct CSV path (legacy mode)
        input_file = Path(weeks_arg[0])
        if not input_file.exists():
            print(f"Error: {input_file} not found")
            sys.exit(1)
        week_nums = [''.join(c for c in input_file.stem if c.isdigit()) or '1']
        schedules_dir = str(input_file.parent)
        # In legacy path the CSV may be anywhere; override schedules_dir lookup
        # by temporarily copying to expected location — just call process directly
        library_path = Path(args.library) if args.library else                        Path('workout_library.csv') if Path('workout_library.csv').exists() else                        input_file.parent / 'workout_library.csv'
        ...
        # Process the single explicit file directly
        workouts, week_start_iso = [], None
        with open(input_file) as f:
            reader = csv.DictReader(f)
            for row in reader:
                if week_start_iso is None:
        ...
        if not workouts:
            print("Error: No valid workouts found"); sys.exit(1)

        week_num   = ''.join(c for c in input_file.stem if c.isdigit()) or '1'
        date_range = get_week_date_range(week_num, week_start_iso)
        totals     = calculate_group_totals(workouts)
        html_out   = generate_html(week_num, workouts, totals, library, date_range)
        out_file   = Path(args.out) / (input_file.stem + '.html') \
                     if args.out else Path(__file__).parent / (input_file.stem + '.html')
        with open(out_file, 'w') as f:
            f.write(html_out)
        print(f"✓ Generated {out_file}")
        return

    # Numeric mode — single week or range
    if len(weeks_arg) == 1:
        week_nums = [int(weeks_arg[0])]
    elif len(weeks_arg) == 2 and weeks_arg[0].isdigit() and weeks_arg[1].isdigit():
        start, end = int(weeks_arg[0]), int(weeks_arg[1])
        week_nums = list(range(start, end + 1))
    else:
        # Space-separated list of individual week numbers
        try:
            week_nums = [int(w) for w in weeks_arg if w.isdigit()]
        except ValueError:
            print(f"Could not parse week arguments: {weeks_arg}")
            sys.exit(1)

    # Load workout library once
    library_path = Path(args.library) if args.library else                    Path('workout_library.csv') if Path('workout_library.csv').exists() else                    Path(args.schedules_dir) / 'workout_library.csv'
    library = load_workout_library(library_path)
    if library:
        print(f"✓ Loaded {len(library)} workouts from library")
    else:
    ...
    print(f"\nGenerating {len(week_nums)} week(s) from {args.schedules_dir}\n")

    ok = fail = 0
    for wn in week_nums:
        if process_one_week(str(wn), library, day_map, args.schedules_dir, args.out):
            ok += 1
        else:
            fail += 1

    print(f"\n{'='*50}")
    print(f"  Done — {ok} generated, {fail} skipped")
    print(f"{'='*50}")
