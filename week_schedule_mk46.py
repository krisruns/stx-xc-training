...
def has_parseable_paces(workout_desc):
    ...
    - "12x400@5k", "2x200@mile" (simple patterns)
    - "3x[600@3k + 400@mile + 200@800]" (bracketed patterns)
    """
    if not workout_desc:
        return False
    
    # Check for bracketed patterns: 3x[...]
    bracket_pattern = r'\d+x\[.*?@.*?\]'
    if re.search(bracket_pattern, workout_desc.lower()):
        return True
    
    # Check for simple patterns: [number]x[distance]@[pace_type]
